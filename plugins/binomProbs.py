...
def calc_pull(D_raw, R_list_raw, tol, optAB):
    ...
    prob = np.clip(prob, a_max=1, a_min=np.power(0.1,16))

    #prob = prob.astype(np.float64)

    ## isf(prob,1) == ppf(1-prob,1) but doesn't have precision problems of 1-prob
    pull = np.sqrt(scipy.stats.chi2.isf(prob, 1))#np.sqrt(scipy.stats.chi2.ppf(1-prob,1))

    return pull


#---------------------------------------
# combination of sqrt and binom 
#---------------------------------------
  
#     R_list_raw_tol = R_list_raw * scaleTol 
...
# 
#     prob *= 2  ## Multiply by 2 for 2-sided probability  
#     prob = np.clip(prob, a_max=1, a_min=np.power(0.1,0.9) , a_min=None)
# 
#     pull_lowStat = np.sqrt(sci.stats.chi2.ppf(1-prob,1))
# 
# 
